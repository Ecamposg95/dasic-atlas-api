...
export function HelloPage() {
  ...
  if (isLoading) return <div className="p-6 text-slate-500 dark:text-slate-400">Cargando…</div>;
  if (error) {
    const status = (error as { status?: number })?.status;
    if (status === 401) return null;
    return <div className="p-6 text-rose-600 dark:text-rose-400">Error: {String((error as { detail?: string })?.detail ?? error)}</div>;
  }
