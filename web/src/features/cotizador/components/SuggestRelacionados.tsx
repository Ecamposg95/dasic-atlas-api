...
export function SuggestRelacionados() {
  ...
  return (
    <div className="bg-card border border-border rounded-xl p-3 mt-3">
      <div className="flex items-center gap-2 text-[10px] uppercase tracking-wider font-bold text-muted-foreground mb-2">
      ...
      <div className="flex gap-2 flex-wrap">
        {items.map((p) => (
          <button
            ...
            className="text-xs px-2 py-1 rounded border border-border-strong hover:border-accent-glow text-foreground hover:text-accent-glow flex items-center gap-1 transition disabled:opacity-50 disabled:cursor-wait"
            title={`Co-ocurrencia: ${p.co_apariciones} vez/veces`}
          >
            <Plus className="h-3 w-3" />
            <span className="font-mono">{p.sku || '—'}</span>
            <span className="text-muted-foreground hidden md:inline">· {p.nombre.slice(0, 30)}</span>
          </button>
        ))}
      </div>
    </div>
