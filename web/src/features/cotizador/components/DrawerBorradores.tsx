    ...
    <>
      ...
      <aside className="fixed right-0 top-0 bottom-0 z-50 w-full max-w-md bg-slate-900 border-l border-slate-800 shadow-2xl flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-2">
          {isLoading && (
            <div className="text-xs text-slate-500 text-center p-4">Cargando…</div>
          )}
          ...
          {items.map((b) => (
            <a
              ...
              className="block p-3 rounded border border-slate-800 hover:border-accent-glow bg-slate-950 transition"
            >
        ...
        <div className="p-4 border-t border-slate-800 flex justify-between items-center">
          ...
          <span className="text-[10px] text-slate-500">Página {page}</span>
          <Button
            size="sm"
            variant="ghost"
            disabled={!hayMas}
