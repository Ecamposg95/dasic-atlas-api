  ...
  return (
    <Modal title="Proveedores" onClose={onClose} size="lg">
      <div className="space-y-4 max-h-[60vh] overflow-y-auto pr-1">
        {/* Lista */}
        {isLoading && <div className="text-slate-400 text-sm">Cargando proveedores…</div>}
            ...
            <tbody>
              {(proveedores ?? []).map((p) => (
                <tr key={p.id} className="border-b border-slate-800/50 hover:bg-slate-800/30">
                  <td className="py-1.5 pr-2 font-medium text-slate-200">{p.nombre_empresa}</td>
                  <td className="py-1.5 px-2 text-slate-400">{p.contacto_nombre ?? '—'}</td>
                  <td className="py-1.5 px-2 text-slate-400">{p.telefono ?? '—'}</td>
        ...
        {showForm && (
          <div className="border border-slate-700 rounded-lg p-3 space-y-2 bg-slate-900/50">
            <div className="text-sm font-medium text-slate-200 mb-2">Nuevo proveedor</div>
            <div>
              <label className="block text-xs text-slate-400 mb-1">
                Nombre empresa <span className="text-rose-400">*</span>
              </label>
              <Input
                value={nombre}
